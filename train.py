...
from mixture_of_depths import mod_score
...
def normalize_scores(score_dict, label=""):
    ...
    if spread < 1e-12:
        ...
        print(f"  [WARN] {label}: raw score spread is {spread:.2e} (min={min_s:.6f}, "
              ...
              f"This means mod_score() is returning near-identical values; see "
              f"mixture_of_depths.py's input standardization fix.")
...
def run_for_window(returns, macro_df, window_days, label=""):
    ...
    for ticker in ret_window.columns:
        ...
        if not np.isfinite(s):
            print(f"    [{ticker}] WARNING: mod_score returned non-finite value, "
    ...
    top_etfs = [{"ticker": t, "mod_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/mod_{timestamp}.json"
