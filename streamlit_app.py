...
st.set_page_config(page_title="Mixture of Depths (MoD)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 Mixture of Depths (MoD)</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Dynamic compute allocation | Tokens skip layers based on depth gating</p>', unsafe_allow_html=True)
...
st.sidebar.markdown(f"**Hidden size:** {config.HIDDEN_SIZE} | **Layers:** {config.NUM_LAYERS} | **Heads:** {config.NUM_HEADS}")
st.sidebar.markdown(f"**Seq len:** {config.SEQ_LEN} | **Threshold:** {config.DEPTH_THRESHOLD}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'mod_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>MoD score: {etf['mod_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized MoD Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized MoD Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧠 Top ETFs by Mixture of Depths × Momentum (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Mixture of Depths (MoD)** dynamically allocates compute by allowing tokens to skip layers.
        - A **depth gating** mechanism decides whether each token continues to the next layer.
        - Tokens below the threshold exit early, saving compute.
        - The model learns from sequences of ETF returns and macro variables.
        - **Momentum factor:** 1 + last_return (clipped to 0.5–2.0).
        - **Final score = MoD prediction × momentum** – adapts to changing market conditions.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the MoD scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Mixture of Depths | Dynamic compute allocation with depth gating × momentum")
